...
# Attempt to establish USB connection with Arduino (OUTPUT module)
try:
    arduino = serial.Serial(port='/dev/ttyUSB0', baudrate=9600, timeout=1)
    print("[INIT] Arduino serial connection established.")
...
def read_from_output():
    ...
    while True:
        try:
            ...
            print(f"[ERROR] Exception in reading from Arduino: {e}")
        time.sleep(0.1)
...
def send_to_arduino(largest_boxes):
    try:
        ...
        if arduino:
            ...
            print("[ERROR] Arduino serial connection not available.")
    except Exception as e:
    ...
    for data in largest_boxes.values():
        if data is not None:
            try:
                ...
                print(f"[ERROR] Failed to draw prediction: {e}")
...
def assign_segment(x1, x2):
    ...
    try:
        ...
        print(f"[ERROR] Failed to assign segment: {e}")
        return 0
...
def preprocess_input(image, input_size):
    try:
        ...
        print(f"[ERROR] Preprocessing input failed: {e}")
        return None
...
def process_detections(output_data, input_shape, conf_threshold=0.23, iou_threshold=0.5):
    ...
    try:
        ...
        print(f"[ERROR] Failed to reshape output data: {e}")
        return []
    ...
    for detection in output_data:
        try:
            ...
            print(f"[ERROR] Processing a detection failed: {e}")
    
    # Apply Non-Maximum Suppression (NMS)
    try:
        detections = non_max_suppression(detections, iou_threshold)
    except Exception as e:
        print(f"[ERROR] Non-maximum suppression failed: {e}")
        detections = []
...
def main():
    ...
    print("[CV] Starting video stream from CSI camera...")
    # GStreamer pipeline for CSI camera (OV5647) using libcamera.
    # Adjust the pipeline parameters (width, height, framerate) as necessary.
    gst_pipeline = ("libcamerasrc ! video/x-raw,width=720,height=480,framerate=30/1 ! videoconvert ! appsink")
    try:
        video_stream = cv2.VideoCapture(gst_pipeline, cv2.CAP_GSTREAMER)
    except Exception as e:
        print(f"[ERROR] Failed to open CSI camera with GStreamer pipeline: {e}")
        return

    if not video_stream.isOpened():
        print("[ERROR] CSI camera not detected or pipeline failed to open.")
        return
    ...
    try:
        ...
        print(f"[ERROR] Failed to load or allocate TFLite model: {e}")
        return
    ...
    while True:
        try:
            ...
            if not ret:
                print("[ERROR] Failed to grab frame from CSI camera.")
                break
            ...
            if input_tensor is None:
                print("[ERROR] Input preprocessing failed. Skipping frame.")
                continue
            ...
            try:
                detections = process_detections(output_data, (input_size, input_size, 3), conf_threshold=0.23, iou_threshold=0.5)
            except Exception as e:
            ...
            for detection in detections:
                try:
                    ...
                    print(f"[ERROR] Failed to process a detection: {e}")
            ...
            try:
                ...
                print(f"[ERROR] Failed to display video frame: {e}")
    ...
    print("[CV] Cleaned up and closed all resources.")
