...
# Display configuration for 720p resolution
display_width = 1280    # 720p width
display_height = 720    # 720p height
...
def main():
    # Start video stream using the CSI camera on the RPi5 (CSI/DSP Port 0)
    print("[CV] Starting video stream from CSI camera...")
    pipeline = ("libcamerasrc sensor-id=0 ! video/x-raw,width=1280,height=720,framerate=45/1 ! "
                "videoconvert ! appsink")
    video_stream = cv2.VideoCapture(pipeline, cv2.CAP_GSTREAMER)
    if not video_stream.isOpened():
        print("[CV] Failed to open video stream.")
        return
