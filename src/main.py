...
def send_to_arduino(largest_boxes):
    ...
    # Use a space as delimiter to match the Arduino's expected format
    message = " ".join(map(str, classes_message)) + "\n"
    arduino.write(message.encode())  # Send the complete message to Arduino
    # print(f"Sent: {message.strip()}")  # Optional: Log the sent message

def display_pred(img, largest_box):
    # Displays bounding box and label on the image
...
def assign_segment(x1, x2):
    # Assigns object to an image segment based on its horizontal position
    x = ((x2 - x1) / 2) + x1
