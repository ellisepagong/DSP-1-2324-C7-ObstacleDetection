...
def main():
    ...
    try:
        # For Raspberry Pi, use "/dev/ttyUSB0" (update if needed)
    ...
    with dai.Device(pipeline) as device:
        ...
        while True:
            ...
            # Combine the RGB feed and depth heatmap into one frame (side-by-side)
            combined_frame = np.hstack((rgbFrame_disp, depth_colormap))
            # Save the combined frame into the "inferences" folder with a timestamp
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            inference_filename = inferences_dir / f"inference_{timestamp}.png"
            cv2.imwrite(str(inference_filename), combined_frame)
